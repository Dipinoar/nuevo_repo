...
echo "============================================================"
echo "[INFO] DEPLOY QA AAB TO GOOGLE PLAY (internal track)"
echo "[INFO] QUALITY_PACKAGE: ${QUALITY_PACKAGE:-NOT SET}"
...
if [ -z "${QUALITY_PACKAGE:-}" ]; then
	echo "[ERROR] QUALITY_PACKAGE is not set. Configure this CI/CD variable."
	exit 1
fi
...
# ====== Localizar AAB de QA ======
echo "[INFO] Locating QA AAB in ${RELEASE_PATH}"
AAB_FILE=$(find "${RELEASE_PATH}" -name "*gms*quality*.aab" -type f | head -1)

if [ -z "${AAB_FILE}" ]; then
	echo "[ERROR] No quality AAB found in ${RELEASE_PATH}"
...
if [ "${GOOGLE_PLAY_DRY_RUN:-false}" == "true" ]; then
	...
	echo "[INFO] Uploading QA AAB to Google Play (track: ${INTERNAL_TRACK})"
	fastlane supply \
		--aab "${AAB_FILE}" \
		--package_name "${QUALITY_PACKAGE}" \
		--track "${INTERNAL_TRACK}" \
		...
		--skip_upload_images \
		--skip_upload_screenshots
	echo "[INFO] Successfully uploaded QA AAB to Google Play (track: ${INTERNAL_TRACK})"
fi

echo "============================================================"
echo "[INFO] deploy_qa_aab_google_play.sh finished OK"
echo "============================================================"
