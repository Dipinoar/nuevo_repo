...
# ====== Resolve desired HMS Gradle task ======
DESIRED_TASK=""

case "${TARGET_BRANCH}" in
	development)
		[[ "${BUILD_TYPE_MODE}" == "env" ]] && DESIRED_TASK="assembleHmsDevelopment" || DESIRED_TASK="assembleHmsDebug"
		;;
	quality)
		[[ "${BUILD_TYPE_MODE}" == "env" ]] && DESIRED_TASK="assembleHmsQuality" || DESIRED_TASK="assembleHmsHomolog"
		;;
	production)
		[[ "${BUILD_TYPE_MODE}" == "env" ]] && DESIRED_TASK="assembleHmsProduction" || DESIRED_TASK="assembleHmsRelease"
		;;
	*)
		[[ "${BUILD_TYPE_MODE}" == "env" ]] && DESIRED_TASK="assembleHmsDevelopment" || DESIRED_TASK="assembleHmsDebug"
		;;
...
# ====== Validate HMS Gradle task ======
echo "[INFO] Resolving available assembleHms tasks from Gradle..."
AVAILABLE_TASKS=$(./gradlew tasks --all --quiet | grep -oE 'assembleHms[A-Za-z]+' | sort -u || true)
...
if echo "${AVAILABLE_TASKS}" | grep -qx "${DESIRED_TASK}"; then
	...
	case "${DESIRED_TASK}" in
		assembleHmsDevelopment) FALLBACK_TASK="assembleHmsDebug" ;;
		assembleHmsQuality) FALLBACK_TASK="assembleHmsHomolog" ;;
		assembleHmsProduction) FALLBACK_TASK="assembleHmsRelease" ;;
		assembleHmsDebug) FALLBACK_TASK="assembleHmsDevelopment" ;;
		assembleHmsHomolog) FALLBACK_TASK="assembleHmsQuality" ;;
		assembleHmsRelease) FALLBACK_TASK="assembleHmsProduction" ;;
		*) FALLBACK_TASK="" ;;
	esac
	...
	if [[ -n "${FALLBACK_TASK}" ]] && echo "${AVAILABLE_TASKS}" | grep -qx "${FALLBACK_TASK}"; then
		...
		echo "[ERROR] No valid assembleHms task found"
		exit 1
	fi
fi
...
echo "parametrized_build_huawei.sh finished OK"
